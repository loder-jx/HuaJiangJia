        ...
        <div class="user-content" :class="{ 'content-hidden': !avatarLoaded }">
            ...
            <div class="user-info">
                <div class="user-main">
                    ...
                    <div class="user-id">花匠家号：{{ user.userId }}</div>
...
const isCurrentUser = computed(() => {
    ...
    const currentUserId = userStore.userInfo.user_id // 当前用户的花匠家号
    const userId = props.user.user_id || props.user.userId // 用户的花匠家号
...
const userHoverConfig = computed(() => ({
    ...
    onFollow: (userInfo) => {
        const userId = props.user.user_id || props.user.userId
        handleFollow(userId) // 使用花匠家号进行关注操作
    },
    onUnfollow: (userInfo) => {
        const userId = props.user.user_id || props.user.userId
        handleUnfollow(userId) // 使用花匠家号进行取消关注操作
    },
    delay: 500
}))
</script>
