...
router.get('/check-user-id', async (req, res) => {
  try {
    const { user_id } = req.query; // 前端传过来的花匠家号
    if (!user_id) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: '请输入花匠家号' });
    }
    // 查数据库是否已有该ID
    ...
    res.json({
      ...
      message: existingUser.length > 0 ? '花匠家号已存在' : '花匠家号可用'
    });
  } catch (error) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    if (user_id.length < 3 || user_id.length > 15) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: '花匠家号长度必须在3-15位之间' });
    }

    if (!/^[a-zA-Z0-9_]+$/.test(user_id)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json({ code: RESPONSE_CODES.VALIDATION_ERROR, message: '花匠家号只能包含字母、数字和下划线' });
    }
    ...
    console.log(`用户注册成功 - 用户ID: ${userId}, 花匠家号: ${userRows[0].user_id}`);
...
router.post('/login', async (req, res) => {
  try {
    ...
    console.log(`用户登录成功 - 用户ID: ${user.id}, 花匠家号: ${user.user_id}`);
